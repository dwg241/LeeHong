...
public class PhoneBookMain5 {
	static Phonemanger[] pBook =new Phonemanger[100];
	...
	public static void insert() {
		...
		if(bDay != null && bDay.trim().length()>0) {
		 pi = new Phonemanger(name, pNum, bDay);
		}
		else {
	     pi = new Phonemanger(name, pNum, bDay);	
		}
	   ...
	   pBook[cnt] =pi;
		...
		if(choice==1){
		    ...
		    String uni=kb.nextLine();
		    System.out.print("전공을 입력해주세요! ");
		    String maj=kb.nextLine();
		    
		    pBook[cnt++] = new Phonemanger(name, pNum, bDay,uni,maj);
		
		}
		if(choice==2){
			System.out.print("직장을 입력해주세요! "); 
		    String job=kb.nextLine();
		    System.out.print("직장 연락처를 입력해주세요! ");
		    String jobNum=kb.nextLine();
		    
		    pBook[cnt++] =  new Phonemanger(name, pNum, bDay, job ,jobNum);
