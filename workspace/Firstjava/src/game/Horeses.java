...
public class Horeses {
	...
	void MoneyCount() {
        	...
        	for(int i=1; i<6; i++)
        	{        	  
        	if(i==1) HoresMoney[k][i]=(int)(HoresMoney[k][i]*4*rank[k]);
        	if(i==2) HoresMoney[k][i]=(int)(HoresMoney[k][i]*2*rank[k]);
        	if(i==3) HoresMoney[k][i]=(int)(HoresMoney[k][i]*1.5*rank[k]);
        	if(i==4) HoresMoney[k][i]=(int)(HoresMoney[k][i]*1.25*rank[k]);
        	if(i==5) HoresMoney[k][i]=(int)(HoresMoney[k][i]*1.1*rank[k]);
        	}
	...
	void Moneychart(int times)
	{
		...
		{
		System.out.println("======== "+times+" Round 베팅 현황========");
		System.out.println("**나의 베팅판**      **전체 베팅판**");
		}
		for(int i=1 ; i<7;i++) {
